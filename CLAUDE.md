...
## AI 智能体行为与协作规范 (Agent Behavior & Workflow)

### 1. 文档优先，严禁主观臆断

- 涉及第三方库、框架（如 LangGraph、LangChain、Elysia、React 等）的 API、数据结构或参数时，严禁凭历史记忆臆断。
- 必须优先查阅本地技能（`.agents/skills/`）或使用 `ctx7` 检索最新官方文档。
- 给出复杂结构（如流式数据包、事件模型）前，须通过文档或轻量运行验证其真实结构。

### 2. 尊重自主编码，禁止擅自修改文件

- 用户处于主动学习与亲自编码阶段。
- 未经用户明确许可或明确要求（如“帮我写进去”、“修改某文件”），严禁擅自调用工具修改项目代码文件。
- AI 的主要职责是答疑解惑、分析原理、指出 Bug 根源并给出参考示例，由用户自行编写与落地。
